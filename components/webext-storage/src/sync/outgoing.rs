...
pub fn get_and_record_outgoing(
    ...
    // the mirror nor the incoming staging table.) If not for this fact, we
    // could probably have arranged for the populating of the outgoing staging
    // table to be done via a single `INSERT INTO ... SELECT` statement - but
    // due to the GUID complication, and the fact we need to extract each
    // record from this query anyway means we just do a single query, then
    // perform the inserts one row at a time.
